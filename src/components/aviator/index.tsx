...
import AutoBetModal from './AutoBetModal'; // ✅ Added
  ...
  useEffect(() => {
    Assets.load(urls).then(() => {
      (async () => {
        ...
        if (!token) return setAviatorState(prev => ({ ...prev, auth: false }));
        ...
        try {
          ...
          localStorage.setItem(
            'bet6',
            JSON.stringify(configResponse.data.data.config.chips.slice(0, 6).map((item: number) => `${item}`))
          );
