...
import RuleModal from '../RuleModal'          // relative import fixed
        ...
        Assets.load(urls).then(() => {
            (async () => {
                ...
                try {
                    ...
                    localStorage.setItem('bet6', JSON.stringify(configResponse.data.data.config.chips.slice(0,6).map((item:number) => `${item}`)))
